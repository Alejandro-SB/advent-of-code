...
namespace AdventOfCode2019Tests
{
    public class Day3Tests
    {
        ...
        public void CalculatesCrossingDistance(string path1, string path2, int expectedDistance)
        {
            var distance = Day3.GetCrossingDistanceForPaths(path1, path2);
        ...
        public void CalculatesMinimumTiming(string path1, string path2, int expectedTiming)
        {
            var timing = Day3.GetMinimumTimingForPaths(path1, path2);

            Assert.Equal(expectedTiming, timing);
        }
    }
}